...
function CaptureVotes() {
  ...
  function handleVoteClicked() {
    console.log(
      `Vote clicked: election selected ${electionSelected}, ID ${idValue}`
    );
    setView(VIEW.VOTING);
  }
  ...
  function renderFormBody() {
    switch (view) {
      ...
      case VIEW.VOTING:
        return (
          <VotesCastingForm
            electionId={electionSelected}
            userId={idValue}
            handleCastVoteClicked={handleCastVoteClicked}
          />
        );
      case VIEW.SUCCESS:
